...
function isMyWorklistRole(qualStr) {
  const s = String(qualStr || '').trim();
  // fail-safe（2026-07-31，feifaren 团队事故）：资质串【读不到】(空/旧版字段没解析到)时
  //   一律【保留进清单、交 case 里判归属】——绝不让 feifaren 因"读不到就不命中关键词"而【静默丢件】
  //   （实测：该审的商标注册证被 feifaren 漏掉、不设角色却能看到）。有值才按角色/「其它」判。
  if (!s) return true;
