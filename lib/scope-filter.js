...
function isInMyRole(sealTypeStr) {
  ...
  // faren = 只看法人词根（含cross-type：同时有法人+非法人的归faren）
  if (ROLE === 'faren') return hasFaren;
  // feifaren = 只看非法人词根，且不能同时有法人词根（cross-type归faren）
  if (ROLE === 'feifaren') return hasFeifaren && !hasFaren;
  return true;
}

module.exports = { isInScope, isInMyRole, MY_AUDIT_QUALS, FAREN_KEYWORDS, FEIFAREN_KEYWORDS };
