...
// 静默探测（2026-08-01 抽出）：返回 soffice 路径或 null，【无副作用日志】——单一事实源的路径清单，
//   供 doctor 等诊断场景静默复用（doctor 现被首跑 list 调用，绝不能带 console.error 噪音）。
function detectSofficeBin() {
...
// 模块加载时的探测：复用静默探测，miss 才打日志（保留既有部署行为不变）。
function resolveSofficeBin() {
  const bin = detectSofficeBin();
  if (bin) return bin;
  const envPath = process.env.QUAL_SOFFICE_BIN;
  if (envPath) {
